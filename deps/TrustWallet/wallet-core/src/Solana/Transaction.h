...
template <typename T> Data shortVecLength(std::vector<T> vec) {
    auto bytes  = Data();
...
struct Instruction {
    ...
    static Instruction createAccountWithSeed(const std::vector<AccountMeta> &accounts, uint64_t value, uint64_t space,
                                             const Address &programId, const Address &voteAddress, uint64_t seedLength,
...
struct CompiledInstruction {
    ...
    /// Supplied address vector is expected to contain all addresses and programId from the instruction; they are
    /// replaced by index into the address vector.
    static CompiledInstruction Compile(const Instruction &instruction, const std::vector<Address> &addresses);
};
...
class Hash {
  public:
    ...
    static Hash fromBase58(const std::string &string) {
        const auto data = Base58::bitcoin.decode(string);
        auto bytes      = std::array<uint8_t, Hash::size>();
...
class Message {
  public:
    ...
    Message() : recentBlockhash(Hash::fromBase58(NULL_ID_ADDRESS)){};
    ...
    static Message createStake(const Address &signer, const Address &stakeAddress, const Address &voteAddress,
        ...
        auto initializeInstruction = Instruction::createStakeInitialize(
            std::vector<AccountMeta>{AccountMeta(stakeAddress, false, false), AccountMeta(sysvarRentId, false, true)},
            signer);
        ...
        auto delegateInstruction = Instruction::createStake(
            ...
            std::vector<AccountMeta>{
                ...
                AccountMeta(sysvarStakeHistoryId, false,
                            true), // 3. `[]` Stake history sysvar that carries stake warmup/cooldown history
    ...
    static Message createStakeDeactivate(const Address &signer, const Address &stakeAddress, Hash recentBlockhash) {
        ...
        auto instruction   = Instruction::createStake(
              Deactivate, std::vector<AccountMeta>{
    ...
    static Message createStakeDeactivateAll(const Address &signer, const std::vector<Address> &stakeAddresses,
                                            Hash recentBlockhash) {
        auto sysvarClockId = Address(SYSVAR_CLOCK_ID_ADDRESS);
        std::vector<Instruction> instructions;
    ...
    static Message createStakeWithdraw(const Address &signer, const Address &stakeAddress, uint64_t value,
                                       Hash recentBlockhash) {
        auto sysvarClockId        = Address(SYSVAR_CLOCK_ID_ADDRESS);
        auto sysvarStakeHistoryId = Address(SYSVAR_STAKE_HISTORY_ID_ADDRESS);
    ...
    static Message createTokenCreateAccount(const Address &signer, const Address &otherMainAccount,
                                            const Address &tokenMintAddress, const Address &tokenAddress,
                                            Hash recentBlockhash) {
        auto sysvarRentId    = Address(SYSVAR_RENT_ID_ADDRESS);
    ...
    static Message createTokenTransfer(const Address &signer, const Address &tokenMintAddress,
                                       const Address &senderTokenAddress, const Address &recipientTokenAddress,
                                       uint64_t amount, uint8_t decimals, Hash recentBlockhash, std::string memo = "",
    ...
    static Message createTokenCreateAndTransfer(const Address &signer, const Address &recipientMainAddress,
                                                const Address &tokenMintAddress, const Address &recipientTokenAddress,
                                                const Address &senderTokenAddress, uint64_t amount, uint8_t decimals,
...
class Transaction {
  public:
    ...
    // The message to sign
    Message message;
    Transaction(const Message &message, const std::vector<Signature> &signatures)
        : message(message), signatures(signatures) {}
  ...
  public:
    static Transaction decode(const Data &raw);
    std::vector<uint8_t> serialize() const;
    std::vector<uint8_t> messageData() const;
    uint8_t getAccountIndex(Address publicKey) const;
...
// auto rawMessageData =
//     ""
// auto expectedHex =
//     "016e7f8349977b482bccf0bfc202ad917295803831e59ccb865b97d657464791ebfe3336879b84b9f165e464a34751fe30d54b01f3c9f33f969aafe1e85951b109"
//     ""
