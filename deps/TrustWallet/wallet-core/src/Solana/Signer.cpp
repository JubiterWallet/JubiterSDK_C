...
bool Signer::verify(const std::vector<PublicKey> &pubKeys, const Transaction &transaction) {
    try {

        for (auto &key : pubKeys) {
            transaction.getAccountIndex(Address{key});
        }
        for (auto i = 0; i < transaction.signatures.size(); i++) {
            auto addr     = transaction.message.accountKeys.at(i);
            auto key      = PublicKey{addr.vector(), TWPublicKeyTypeED25519};
            auto sig      = transaction.signatures.at(i);
            auto sigBytes = TW::Data{sig.bytes.begin(), sig.bytes.end()};
            return key.verify(sigBytes, transaction.messageData());
        }
    } catch (...) {
        return false;
    };
...
//                     stakeAddress = StakeProgram::addressFromRecentBlockhash(userAddress, blockhash,
//                     stakeProgramId);
//                 } else {
...
//                 tokenAddress, blockhash); signerKeys.push_back(key);
//             }
//             break;
//
...
//                 auto decimals = static_cast<uint8_t>(protoMessage.decimals());
//                 const auto memo = protoMessage.memo();
//                 message = Message::createTokenCreateAndTransfer(userAddress, recipientMainAddress,
//                 tokenMintAddress, recipientTokenAddress, senderTokenAddress, amount, decimals, blockhash,
//                     memo, convertReferences(protoMessage.references()));
//                 signerKeys.push_back(key);
//             }
//             break;
...
void Signer::signUpdateBlockhash(const std::vector<PrivateKey> &privateKeys, Transaction &transaction,
