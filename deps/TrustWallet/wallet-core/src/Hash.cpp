...
Data Hash::blake2b(const byte *data, size_t dataSize, size_t hashSize, const Data &personal) {
    Data result(hashSize);
    ::blake2b_Personal(data, static_cast<uint32_t>(dataSize), personal.data(), personal.size(), result.data(),
                       hashSize);
    return result;
}
...
uint64_t Hash::xxhash(const byte *data, size_t size, uint64_t seed) { return XXHash64::hash(data, size, seed); }
...
Data Hash::hmac256(const Data &key, const Data &message) {
    ...
    hmac_sha256(key.data(), static_cast<uint32_t>(key.size()), message.data(), static_cast<uint32_t>(message.size()),
                hmac.data());
    return hmac;
}

Data Hash::hash_tag(const char *tag, const byte *data, size_t size) {
    // sha256(sha256(tag)||sha256(tag)||data)
    Data result(SHA256_DIGEST_LENGTH);
    auto tagDigest = sha256((const byte *)tag, strlen(tag));
    SHA256_CTX ctx;
    sha256_Init(&ctx);
    sha256_Update(&ctx, tagDigest.data(), tagDigest.size());
    sha256_Update(&ctx, tagDigest.data(), tagDigest.size());
...
Data Hash::hash_TapTweak(const byte *data, size_t size) { return hash_tag("TapTweak", data, size); }
