...
class Script {
  public:
    ...
    template <typename It> Script(It begin, It end) : bytes(begin, end) {}
    ...
    /// Builds a pay-to-taproot-key-path-spending
    static Script buildPayToTaprootKeyPathSpending(const Data &program);
    ...
    static std::vector<Data> buildPayToTaprootKeyPathSpendingWitness(const Data &signature);

    // JuBiter-defined
    /// Builds a return0 script from a script.
    static Script buildReturn0(const Data &data, const Data &check = {}, int offset = 0);
    ...
    // JuBiter-defined
    Script &operator+=(const Script &rhs) { return (*this + rhs); }
