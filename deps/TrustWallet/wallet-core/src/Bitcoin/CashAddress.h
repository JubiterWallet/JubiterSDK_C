...
class CashAddress {
  public:
    ...
    CashAddress(std::string hrp, std::vector<uint8_t> prog)
        : hrp(std::move(hrp)), program(std::move(prog)) {}
    ...
    // JuBiter-defined
    /// Decodes a Cash address.
    ///
    /// \returns a pair with the address and a success flag.
    static std::pair<CashAddress, bool> decode(const std::string& addr);
