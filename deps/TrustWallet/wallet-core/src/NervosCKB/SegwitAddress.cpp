...
SegwitAddress::SegwitAddress(const PublicKey& publicKey, int codeHashIndex, std::string hrp)
    : hrp(std::move(hrp)), format(TWNervosCKBAddressFormat::TWNervosCKBAddressShort), codeHashIndex(codeHashIndex), codeHash(), args() {
    if (publicKey.type != TWPublicKeyTypeSECP256k1) {
        throw std::invalid_argument("SegwitAddress needs a compressed SECP256k1 public key.");
    }
    args.resize(TW::Hash::ripemdSize);
    ecdsa_get_pubkeyhash(publicKey.compressed().bytes.data(), HASHER_BLAKE2B,
                         args.data());
}
...
std::pair<SegwitAddress, bool> SegwitAddress::fromRaw(const std::string& hrp,
    ...
    if (TWNervosCKBAddressFormat::TWNervosCKBAddressShort == conv[0]
        ...
        return std::make_pair(SegwitAddress(hrp, conv[0], TW::Data(conv.begin()+1, conv.begin()+1+TW::Hash::sha256Size), TW::Data(conv.begin()+1+TW::Hash::sha256Size, conv.end())), true);
    }
}

std::pair<SegwitAddress, bool> SegwitAddress::fromPublickey(const std::string& hrp,
...
std::pair<SegwitAddress, bool> SegwitAddress::fromPublickey(const std::string& hrp,
    ...
    return SegwitAddress::fromPublickey(hrp,
