...
struct RLP {
    /// Encodes a string;
    static Data encode(const std::string &string) noexcept { return encode(Data(string.begin(), string.end())); }
    ...
    template <std::size_t N> static Data encode(const std::array<uint8_t, N> &data) noexcept {
        Data encoded;
    ...
    template <typename T> static Data encodeList(T elements) noexcept {
    ...
    /// Returns the representation of an integer using the least number of bytes needed, between 1 and 8 bytes, big
    /// endian
