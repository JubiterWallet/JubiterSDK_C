...
//        const Data& to, const uint256_t& amount, const Data& payload = {})
    TransactionNonTyped(const Data& nonce,
        const Data& gasPrice, const Data& gasLimit,
        const Address& to, const Data& amount, const Data& payload = {})
...
public:
    ...
    TransactionPersonal(const Data& bytestring)
    : TransactionBase({}, bytestring) {}
...
//    TransactionTyped(TransactionType type, const uint256_t& nonce, const Data& payload)
    TransactionTyped(TransactionType type, const Data& nonce, const Data& payload, const std::string& accessListInJSON="")
    : TransactionBase(nonce, payload), type(type), accessList(jsonToAccessList(accessListInJSON)) {}
...
protected:
    ...
    // JuBiter-defined
    // conver accessListInJSON to std::vector<Access>
    virtual std::vector<Access> jsonToAccessList(const std::string& accessListInJSON);
...
public:
    // JuBiter-defined
    bool isValid() const {
        if (!(TxType_OptionalAccessList == type
                     || TxType_Eip1559 == type)
            ) {
            return false;
        }
...
public:
    ...
    TransactionOptionalAccessList(const Data& nonce,
        const Data& gasPrice, const Data& gasLimit,
        const Address& to, const Data& amount, const Data& payload = {}, const std::string& accessListInJSON="")
        : TransactionTyped(TxType_OptionalAccessList, nonce, payload, accessListInJSON)
        , gasPrice(std::move(gasPrice))
        , gasLimit(std::move(gasLimit))
        , to(std::move(to))
        , amount(std::move(amount)) {}
...
//        const Data& to, const uint256_t& amount, const Data& payload = {})
    TransactionEip1559(const Data& nonce,
        const Data& maxInclusionFeePerGas, const Data& maxFeePerGas, const Data& gasLimit,
        const Address& to, const Data& amount, const Data& payload = {}, const std::string& accessListInJSON="")
