...
Signature Signer::sign(const PrivateKey &privateKey, const Data &hash, bool includeEip155,
                       const Data &chainID) noexcept {
    auto signature = privateKey.sign(hash, TWCurveSECP256k1);
    return signatureDataToStruct(signature, includeEip155, chainID);
}

bool Signer::verify(const PublicKey &publicKey, const Data &hash, bool includeEip155, const Data &chainID,
...
//                             /* optional data: */ Data(input.transaction().transfer().data().begin(),
//                             input.transaction().transfer().data().end()));
//
...
//                             /* data */ Data(input.transaction().erc1155_transfer().data().begin(),
//                             input.transaction().erc1155_transfer().data().end()));
//
//                     case Proto::TransactionMode::Enveloped: // Eip1559
...
//                             /* transaction: */ Data(input.transaction().contract_generic().data().begin(),
//                             input.transaction().contract_generic().data().end()));
//
//                     case Proto::TransactionMode::Enveloped: // Eip1559
//                         return TransactionEip1559::buildNativeTransfer(
//                             nonce, maxInclusionFeePerGas, maxFeePerGas, gasLimit,
...
Signature Signer::sign(const PrivateKey &privateKey, const Data &chainID,
                       std::shared_ptr<TransactionBase> transaction) noexcept {
...
bool Signer::verify(const PublicKey &publicKey, const Data &chainID, std::shared_ptr<TransactionBase> transaction,
                    const Data &signature) noexcept {
    auto preHash = transaction->preHash(chainID);
...
Signature Signer::sign(const PrivateKey &privateKey, const Data &bytestring) noexcept {
    ...
    return Signer::sign(privateKey, preHash, personal.usesReplayProtection(), {}); // `v` is not depend chainId
}
...
bool Signer::verify(const PublicKey &publicKey, const Data &bytestring, const Data &signature) noexcept {
    TransactionPersonal personal(bytestring);
    auto preHash = personal.preHash();
    return Signer::verify(publicKey, preHash, personal.usesReplayProtection(), {}, // `v` is not depend chainId
                          signature);
}

// JuBiter-defined
/// Signs the given typedData (Eip712).
Signature Signer::sign(const PrivateKey &privateKey, const std::string &jsonData,
                       const bool &bMetamaskV4Compat) noexcept {
    TransactionTypedData typedData(jsonData, bMetamaskV4Compat);
    auto preHash = typedData.preHash();
    return Signer::sign(privateKey, preHash, typedData.usesReplayProtection(), {}); // `v` is not depend chainId
}

// JuBiter-defined
/// Verify the given signature of a typedData (Eip712).
bool Signer::verify(const PublicKey &publicKey, const std::string &jsonData, const Data &signature,
                    const bool &bMetamaskV4Compat) noexcept {
    TransactionTypedData typedData(jsonData, bMetamaskV4Compat);
    auto preHash = typedData.preHash();
    return Signer::verify(publicKey, preHash, typedData.usesReplayProtection(), {}, // `v` is not depend chainId
                          signature);
}
