...
//	SHA256((pbegin == pend ? pblank : (unsigned char*)&pbegin[0]), (pend - pbegin) * sizeof(pbegin[0]), hash1);
	sha256_context ctx;
	memset(&ctx, 0, sizeof(ctx) / sizeof(sha256_context));
	sha256_starts(&ctx, 0);
	if (pbegin == pend) {
...
//    SHA256((pbegin == pend ? pblank : (unsigned char*)&pbegin[0]), (pend - pbegin) * sizeof(pbegin[0]), hash1);
	...
	memset(&sha256Ctx, 0, sizeof(sha256Ctx) / sizeof(sha256_context));
	sha256_starts(&sha256Ctx, 0);
	if (pbegin == pend) {
...
//    RIPEMD160(hash1, sizeof(hash1), hash2);
	ripemd160_context ripemdCtx;
	ripemd160_starts(&ripemdCtx);
	ripemd160_update(&ripemdCtx, hash1, hashlen1);
	ripemd160_finish(&ripemdCtx, hash2);
