...
JUB_RV JubiterLiteCKBImpl::SelectApplet() {

    return JubiterLiteImpl::SelectApplet();
}


JUB_RV JubiterLiteCKBImpl::SetUnit(const JUB_ENUM_BTC_UNIT_TYPE& unit) {

    return JUBR_OK;
}


JUB_RV JubiterLiteCKBImpl::SetCoin(const JUB_ENUM_COINTYPE_BTC& type) {

    return JUBR_OK;
}


JUB_RV JubiterLiteCKBImpl::GetAddress(const JUB_BYTE addrFmt, const JUB_ENUM_BTC_TRANS_TYPE& type, const std::string& path, const JUB_UINT16 tag, std::string& address, const TWCoinType& coinNet) {
...
JUB_RV JubiterLiteCKBImpl::GetHDNode(const JUB_ENUM_BTC_TRANS_TYPE& type, const std::string& path, std::string& pubkey, const TWCoinType& coinNet) {
    ...
    pubkey = uchar_vector(publicKey).getHex();

    return JUBR_OK;
}


JUB_RV JubiterLiteCKBImpl::_SignTX(const std::vector<std::string>& vInputPath,
...
JUB_RV JubiterLiteCKBImpl::_VerifyTX(const std::vector<std::string>& vInputPath,
