...
JUB_RV TrezorCryptoHCImpl::MnemonicToSeed(const std::string &passphrase, const std::string &mnemonic,
    ...
    uchar_vector v;
    TrezorCryptoBTCImpl::MnemonicToSeed(passphrase, mnemonic, v, progress_callback);
...
JUB_RV TrezorCryptoHCImpl::SignTX(const JUB_BYTE addrFmt, const JUB_ENUM_BTC_TRANS_TYPE &type,
...
JUB_RV TrezorCryptoHCImpl::_HdnodeCkd(const std::string &path, HDNode *node, JUB_UINT32 *parentFingerprint,
    ...
    if (JUB_SoftwareTokenType::PRIVATE == _type) {
        ...
        return hdnode_priv_ckd(dprv, path, _curve_name, TWCoinType2HDVersionPublic(_coin),
                               TWCoinType2HDVersionPrivate(_coin), node, parentFingerprint);
    }
    ...
    if (JUB_SoftwareTokenType::PUBLIC == _type) {
        ...
        return hdnode_pub_ckd(dpub, path, _curve_name, TWCoinType2HDVersionPublic(_coin),
                              TWCoinType2HDVersionPrivate(_coin), node, parentFingerprint);
    }
...
JUB_RV TrezorCryptoHCImpl::_xpub2dpub(const std::string &xpub, std::string &dpub) {
    ...
    rv = hdnode_deserialize(xpub.c_str(), xpubPrefix, xprvPrefix, SECP256K1_NAME, &hdkey, &parentFingerprint);
    if (JUBR_OK == rv) {
        // xpub -> dpub
