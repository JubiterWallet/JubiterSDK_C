...
namespace jub {
	namespace token {
		...
		JUB_RV TrezorCryptoEOSImpl::SelectAppletEOS() {
			return JUBR_OK;
		}


		JUB_RV TrezorCryptoEOSImpl::SetCoinTypeEOS() {
			return JUBR_OK;
		}

		JUB_RV TrezorCryptoEOSImpl::GetAddressEOS(const TW::EOS::Type& type, const std::string& path, const JUB_UINT16 tag, std::string& address) {
		...
		JUB_RV TrezorCryptoEOSImpl::GetHDNodeEOS(const JUB_BYTE format, const std::string& path, std::string& pubkey) {
			HDNode hdkey;
			JUB_UINT32 parentFingerprint;
			JUB_VERIFY_RV(hdnode_priv_ckd(_MasterKey_XPRV, path, SECP256K1_NAME, defaultXPUB, defaultXPRV, &hdkey, &parentFingerprint));
		...
		JUB_RV TrezorCryptoEOSImpl::SignTXEOS(const TW::EOS::Type& type,
