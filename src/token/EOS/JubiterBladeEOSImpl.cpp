...
namespace jub {
	namespace token {
		...
		JUB_RV JubiterBladeEOSImpl::SelectAppletEOS() {
			SWITCH_TO_EOS_APP;
			return JUBR_OK;
		}

		//MISC functions
		JUB_RV JubiterBladeEOSImpl::SetCoinTypeEOS() {
		...
		JUB_RV JubiterBladeEOSImpl::GetAddressEOS(const TW::EOS::Type& type, const std::string& path, const JUB_UINT16 tag, std::string& address) {
		...
		JUB_RV JubiterBladeEOSImpl::GetHDNodeEOS(const JUB_BYTE format, const std::string& path, std::string& pubkey) {

			return JUBR_IMPL_NOT_SUPPORT;
		}


		JUB_RV JubiterBladeEOSImpl::SignTXEOS(const TW::EOS::Type& type,
