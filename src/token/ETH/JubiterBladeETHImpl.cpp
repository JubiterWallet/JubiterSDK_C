...
namespace jub {
	namespace token {
		...
		JUB_RV JubiterBladeETHImpl::SelectAppletETH() {
			SWITCH_TO_ETH_APP;
			return JUBR_OK;
		}

		JUB_RV JubiterBladeETHImpl::GetAppletVersionETH(std::string &version) {
		...
		JUB_RV JubiterBladeETHImpl::GetAddressETH(const std::string& path, const JUB_UINT16 tag, std::string& address) {
		...
		JUB_RV JubiterBladeETHImpl::GetHDNodeETH(const JUB_BYTE format, const std::string& path, std::string& pubkey) {
		...
		JUB_RV JubiterBladeETHImpl::SignTXETH(const bool bERC20,
