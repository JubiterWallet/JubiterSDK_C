...
JUB_RV JubiterLiteETHImpl::GetAppletVersion(stVersion &version) { return JubiterLiteImpl::GetAppletVersion(version); }
...
JUB_RV JubiterLiteETHImpl::_encodeRSV(const std::vector<JUB_BYTE> &vRSV, const std::vector<JUB_BYTE> &vChainID,
...
JUB_RV JubiterLiteETHImpl::SignTx(const std::vector<JUB_BYTE> &vNonce, const std::vector<JUB_BYTE> &vGasPrice,
    ...
    try {
        ...
        for (const auto &rsv : vRSV) {
            ...
            JUB_VERIFY_RV(_encodeRSV(rsv, vChainID, signature.r, signature.s, signature.v));
...
JUB_RV JubiterLiteETHImpl::SignTX(const int erc, const std::vector<JUB_BYTE> &vNonce,
...
JUB_RV JubiterLiteETHImpl::VerifyTX(const std::vector<JUB_BYTE> &vChainID, const std::string &path,
                                    const std::vector<JUB_BYTE> &vSigedTrans) {

    TW::Data publicKey;
    JUB_VERIFY_RV(
        JubiterLiteImpl::GetCompPubKey(_getSignType(_curve_name), JUB_ENUM_BTC_TRANS_TYPE::p2pkh, path, publicKey));
...
JUB_RV JubiterLiteETHImpl::SetERC20ETHToken(const std::string &tokenName, const JUB_UINT16 unitDP,
                                            const std::string &contractAddress) {

    return JUBR_OK;
}

JUB_RV JubiterLiteETHImpl::SetERC721ETHToken(const std::string &tokenName, const std::string &contractAddress) {

    return JUBR_OK;
}

JUB_RV JubiterLiteETHImpl::SignContract(const JUB_BYTE inputType, const std::vector<JUB_BYTE> &vNonce,
                                        const std::vector<JUB_BYTE> &vGasPrice, const std::vector<JUB_BYTE> &vGasLimit,
                                        const std::vector<JUB_BYTE> &vTo, const std::vector<JUB_BYTE> &vValue,
                                        const std::vector<JUB_BYTE> &vInput, const std::vector<JUB_BYTE> &vPath,
                                        const std::vector<JUB_BYTE> &vChainID, std::vector<JUB_BYTE> &vRaw) {
...
JUB_RV JubiterLiteETHImpl::SignContractHash(const JUB_BYTE inputType, const std::vector<JUB_BYTE> &vNonce,
...
JUB_RV JubiterLiteETHImpl::SignBytestring(const std::vector<JUB_BYTE> &vData, const std::vector<JUB_BYTE> &vPath,
                                          const std::vector<JUB_BYTE> &vChainID, std::vector<JUB_BYTE> &vSignature) {
    ...
    try {
        ...
        for (const auto &rsv : vRSV) {
            TW::Ethereum::Signature signature;
            JUB_VERIFY_RV(_encodeRSV(rsv, {}, // `v` is not depend chainId
            ...
            std::copy(std::begin(signature.v), std::end(signature.v),
                      std::begin(vSignature) + signature.r.size() + signature.s.size());
        }
    } catch (...) {
        return JUBR_ERROR_ARGS;
    }
...
JUB_RV JubiterLiteETHImpl::VerifyBytestring(const std::string &path, const std::vector<JUB_BYTE> &vData,
...
JUB_RV JubiterLiteETHImpl::SignTypedData(const bool &bMetamaskV4Compat, const std::string &typedDataInJSON,
                                         const std::vector<JUB_BYTE> &vPath, const std::vector<JUB_BYTE> &vChainID,
                                         std::vector<JUB_BYTE> &signatureRaw) {
    return JUBR_IMPL_NOT_SUPPORT;
}

JUB_RV JubiterLiteETHImpl::VerifyTypedData(const bool &bMetamaskV4Compat, const std::string &path,
