...
namespace jub {
	namespace token {
		JUB_RV TrezorCryptoETHImpl::SelectAppletETH() {
			return JUBR_OK;
		}

		JUB_RV TrezorCryptoETHImpl::GetAppletVersionETH(std::string& version) {
			return JUBR_OK;
		}


		JUB_RV TrezorCryptoETHImpl::GetAddressETH(const std::string& path, const JUB_UINT16 tag, std::string& address) {
		...
		JUB_RV TrezorCryptoETHImpl::GetHDNodeETH(const JUB_BYTE format, const std::string& path, std::string& pubkey) {
			HDNode hdkey;
			JUB_UINT32 parentFingerprint;
			JUB_VERIFY_RV(hdnode_priv_ckd(_MasterKey_XPRV, path, SECP256K1_NAME, defaultXPUB, defaultXPRV, &hdkey, &parentFingerprint));
		...
		JUB_RV TrezorCryptoETHImpl::SignTXETH(const bool bERC20,
