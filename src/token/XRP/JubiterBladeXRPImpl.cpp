...
do {                                            \
    JUB_VERIFY_RV(_SelectApp(kPKIAID_MISC, 16));\
		...
		JUB_RV JubiterBladeXRPImpl::SelectAppletXRP() {
			SWITCH_TO_XRP_APP;
			return JUBR_OK;
		}

		//MISC functions
		JUB_RV JubiterBladeXRPImpl::SetCoinTypeXRP() {
		...
		JUB_RV JubiterBladeXRPImpl::GetAddressXRP(const std::string& path, const JUB_UINT16 tag, std::string& address) {
		...
		JUB_RV JubiterBladeXRPImpl::GetHDNodeXRP(const JUB_BYTE format, const std::string& path, std::string& pubkey) {
		...
		JUB_RV JubiterBladeXRPImpl::SignTXXRP(const std::vector<JUB_BYTE>& vPath,
