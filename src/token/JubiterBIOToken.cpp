...
stAppInfos JubiterBIOToken::g_appInfo[] = {
    {TW::Data(uchar_vector(kPKIAID_BTC, sizeof(kPKIAID_BTC) / sizeof(JUB_BYTE))), "BTC", "05000000"},
    {TW::Data(uchar_vector(kPKIAID_ETH, sizeof(kPKIAID_ETH) / sizeof(JUB_BYTE))), "ETH", "05000000"},
    // ETH index position fixed, start adding new apps below:
    {TW::Data(uchar_vector(kPKIAID_ETH, sizeof(kPKIAID_ETH) / sizeof(JUB_BYTE))), "ETC", "05000000"},
    {TW::Data(uchar_vector(kPKIAID_ETH, sizeof(kPKIAID_ETH) / sizeof(JUB_BYTE))), "FIL", "05070001"},
    // BTC index position fixed, start adding new apps below:
    {TW::Data(uchar_vector(kPKIAID_BTC, sizeof(kPKIAID_BTC) / sizeof(JUB_BYTE))), "BCH", "05000000"},
    {TW::Data(uchar_vector(kPKIAID_BTC, sizeof(kPKIAID_BTC) / sizeof(JUB_BYTE))), "LTC", "05000000"},
    {TW::Data(uchar_vector(kPKIAID_BTC, sizeof(kPKIAID_BTC) / sizeof(JUB_BYTE))), "DASH", "05000000"},
    {TW::Data(uchar_vector(kPKIAID_BTC, sizeof(kPKIAID_BTC) / sizeof(JUB_BYTE))), "USDT", "05000000"},
    // MISC applet, start adding new apps below:
    {TW::Data(uchar_vector(kPKIAID_MISC, sizeof(kPKIAID_MISC) / sizeof(JUB_BYTE))), "EOS", "05010000"},
    {TW::Data(uchar_vector(kPKIAID_MISC, sizeof(kPKIAID_MISC) / sizeof(JUB_BYTE))), "XRP", "05010000"},
    {TW::Data(uchar_vector(kPKIAID_MISC, sizeof(kPKIAID_MISC) / sizeof(JUB_BYTE))), "TRX", "05010000"},
    {TW::Data(uchar_vector(kPKIAID_MISC, sizeof(kPKIAID_MISC) / sizeof(JUB_BYTE))), "SOL", "05040000"},
};

JubiterBIOToken::JubiterBIOToken(JUB_UINT16 deviceID) : JubiterBladeToken(deviceID) {}
...
JUB_RV JubiterBIOToken::IdentityVerify(IN JUB_BYTE mode, OUT JUB_ULONG &retry) {
    ...
    if (JUB_ENUM_IDENTITY_VERIFY_MODE::VIA_DEVICE != mode && JUB_ENUM_IDENTITY_VERIFY_MODE::VIA_FPGT != mode) {
...
JUB_RV JubiterBIOToken::IdentityVerifyPIN(IN JUB_BYTE mode, IN const std::string &pinMix, OUT JUB_ULONG &retry) {
    ...
    case JUB_ENUM_IDENTITY_VERIFY_MODE::VIA_9GRIDS: {
        std::transform(pinMix.begin(), pinMix.end(), std::back_inserter(pin),
                       [](const char elem) { return (uint8_t)(elem - 0x30); });
        break;
    }
    case JUB_ENUM_IDENTITY_VERIFY_MODE::VIA_APDU: {
        std::transform(pinMix.begin(), pinMix.end(), std::back_inserter(pin),
                       [](const char elem) { return (uint8_t)elem; });
        break;
    }
    default:
        return JUBR_ARGUMENTS_BAD;
...
JUB_RV JubiterBIOToken::IdentityNineGrids(IN bool bShow) {
    ...
    uchar_vector apduData =
        tlv_buf(0xDFFF, tlv_buf(0x8220, tlv_buf(JUB_ENUM_IDENTITY_VERIFY_MODE::VIA_9GRIDS, {value}).encode()).encode())
            .encode();
    APDU apdu(0x80, 0xCB, 0x80, 0x00, (JUB_ULONG)apduData.size(), apduData.data());
    JUB_UINT16 ret         = 0;
    JUB_BYTE retData[1024] = {
...
JUB_RV JubiterBIOToken::EnrollFingerprint(IN JUB_UINT16 fpTimeout, INOUT JUB_BYTE_PTR fgptIndex,
                                          OUT JUB_ULONG_PTR ptimes, OUT JUB_BYTE_PTR fgptID) {
...
JUB_RV JubiterBIOToken::DeleteFingerprint(IN JUB_UINT16 fpTimeout, JUB_BYTE fgptID) {
