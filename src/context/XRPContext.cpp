...
namespace jub {
	namespace context {
		JUB_RV XRPContext::ActiveSelf() {
			JUB_VERIFY_RV(_tokenPtr->SelectAppletXRP());
			JUB_VERIFY_RV(_tokenPtr->SetCoinTypeXRP());
		...
		JUB_RV XRPContext::GetMainHDNode(JUB_BYTE format, std::string& xpub) {
			...
			JUB_VERIFY_RV(_tokenPtr->GetHDNodeXRP(format, _mainPath, xpub));
		...
		JUB_RV XRPContext::GetAddress(BIP44_Path path, JUB_UINT16 tag, std::string& address) {
			...
			JUB_VERIFY_RV(_tokenPtr->GetAddressXRP(strPath, tag, address));
		...
		JUB_RV XRPContext::SetMyAddress(BIP44_Path path, std::string& address) {
			...
			std::string strPath = _FullBip44Path(path);
			JUB_VERIFY_RV(_tokenPtr->GetAddressXRP(strPath, 0x02, address));
		...
		JUB_RV XRPContext::GetHDNode(JUB_BYTE format, BIP44_Path path, std::string& pubkey) {
			...
			std::string strPath = _FullBip44Path(path);
			JUB_VERIFY_RV(_tokenPtr->GetHDNodeXRP(format, strPath, pubkey));
		...
		JUB_RV XRPContext::SignTransaction(BIP44_Path path,
			...
			try {
				...
				JUB_VERIFY_RV(_tokenPtr->SignTXXRP(vPath,
					vPreimage,
					vSignatureRaw));
