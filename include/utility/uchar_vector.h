...
namespace bitcoin {
	namespace util {
		...
		public:
			...
			uchar_vector& operator<<(opcodetype byte)
			{
			...
			uchar_vector& operator=(const std::string& hex) { this->setHex(hex); m_cur_it = begin(); m_last_op_it = begin(); return *this; }
			...
			uchar_vector& operator | (uchar_vector&  v)
			{
				...
				uint32_t len = 0;
				if (*m_cur_it == OP_PUSHDATA1) {
					m_last_op_it = m_cur_it;
					m_cur_it++;
					len = read_uint8();
				}
				else if (*m_cur_it == OP_PUSHDATA2) {
					m_last_op_it = m_cur_it;
					m_cur_it++;
					len = read_le_uint16();
				}
				else if (*m_cur_it == OP_PUSHDATA4) {
					m_last_op_it = m_cur_it;
					m_cur_it++;
					len = read_le_uint32();
				}
