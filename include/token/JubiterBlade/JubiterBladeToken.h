...
constexpr JUB_BYTE kPKIAID_DOMAIN[8] = {0xA0, 0x00, 0x00, 0x00, 0x03, 0x00, 0x00, 0x00};

constexpr JUB_BYTE kPKIAID_MISC[16] = {0xD1, 0x56, 0x00, 0x01, 0x32, 0x03, 0x00, 0x42,
                                       0x4C, 0x44, 0x00, 0x6D, 0x69, 0x73, 0x63, 0x01};

constexpr JUB_BYTE kPKIAID_FIDO[8] = {0xa0, 0x00, 0x00, 0x06, 0x47, 0x2f, 0x00, 0x01};
...
typedef enum class enumCoinTypeETH { COINETH = 0x00, COINFIL = 0x01, Default = COINETH } JUB_ENUM_COINTYPE_ETH;
...
class JubiterBladeToken : public HardwareTokenInterface {
  ...
  public:
    ...
    virtual JUB_RV SetERC20Tokens(const ERC20_TOKEN_INFO tokens[], const JUB_UINT16 iCount) override;
    virtual JUB_RV SetERC20Token(JUB_CHAR_CPTR tokenName, JUB_UINT16 unitDP, JUB_CHAR_CPTR contractAddress) override;
    virtual JUB_RV SetERC721Token(JUB_CHAR_CPTR tokenName, JUB_CHAR_CPTR contractAddress) override;

    virtual JUB_RV GetHDNode(const JUB_ULONG p1, const JUB_ULONG p2, const std::string &path,
                             TW::Data &pubkey) override;
    virtual JUB_RV GetAddress(const JUB_ULONG p1, const JUB_ULONG p2, const TW::Data &apduData,
                              TW::Data &address) override;

    // NFC
    virtual JUB_RV SetLabel(const std::string &label) override;
    virtual JUB_RV Reset() override;
    virtual JUB_RV GenerateSeed(const std::string &pinMix, const JUB_ENUM_CURVES &curve) override;

    virtual JUB_RV ImportMnemonic(const std::string &pinMix, const std::string &mnemonic) override;
    virtual JUB_RV ExportMnemonic(const std::string &pinMix, OUT std::string &mnemonic) override;
    ...
    virtual JUB_RV EnrollFingerprint(IN JUB_UINT16 fpTimeout, INOUT JUB_BYTE_PTR fgptIndex, OUT JUB_ULONG_PTR ptimes,
    ...
    virtual JUB_RV DeleteFingerprint(IN JUB_UINT16 fpTimeout, JUB_BYTE fgptID) override;

  protected:
    JUB_BYTE _HighMark(const JUB_ULONG &highMark) { return ((highMark & 0x0F) << 4); }

    virtual JUB_RV _SendApdu(const APDU *apdu, JUB_UINT16 &wRet, JUB_BYTE *retData = nullptr,
                             JUB_ULONG *pulRetDataLen = nullptr, JUB_ULONG ulMiliSecondTimeout = 1200000);

    JUB_RV _TranPack(const TW::Data &apduData, const JUB_BYTE highMark, const JUB_BYTE sigType,
                     const JUB_ULONG ulSendOnceLen, int finalData = false, int bOnce = false);

    JUB_RV _TranPackApdu(const JUB_ULONG ncla, const JUB_ULONG nins, const TW::Data &apduData, const JUB_BYTE highMark,
                         const JUB_BYTE sigType, const JUB_ULONG ulSendOnceLen, JUB_BYTE *retData = nullptr,
                         JUB_ULONG *pulRetDataLen = nullptr, int finalData = false, int bOnce = false);
    ...
    virtual JUB_RV _EncodeAppletInfoInJSON(const std::vector<JUB_APPLET_INFO> &appletInfoList,
                                           std::string &appletInfoListInJSON);
