...
class JubiterBLDImpl : public TokenInterface {
   ...
   public:
	...
	virtual JUB_RV getHDNode_ETH(std::string path, std::string& pubkey);
	virtual JUB_RV signTX_ETH(bool Is_ERC20, std::vector<JUB_BYTE> nonce, std::vector<JUB_BYTE> gasLimit, std::vector<JUB_BYTE> gasPrice,
		std::vector<JUB_BYTE> To, std::vector<JUB_BYTE> value, std::vector<JUB_BYTE> data, std::vector<JUB_BYTE> path, std::vector<JUB_BYTE> chainID,
		std::vector<JUB_BYTE>& raw);
