...
class TrezorCryptoToken : public SoftwareTokenInterface {
  public:
    // for Factory
    template <typename T>
    static std::shared_ptr<BaseToken> Create(JUB_UINT16 deviceID,
                                             const JUB_ENUM_CURVES curve = JUB_ENUM_CURVES::SECP256K1) {
  ...
  public:
    TrezorCryptoToken(JUB_UINT16 deviceID, JUB_ENUM_CURVES curve) : _deviceID(deviceID), _curve(curve) {}
    ~TrezorCryptoToken() {}

    virtual JUB_RV
    ToMasterKey(const JUB_ENUM_CURVES &curve, const std::string &privOrPub,
    ...
    virtual JUB_RV MnemonicToMasterPrivateKey(const JUB_ENUM_CURVES &curve, const std::string &passphrase,
                                              const std::string &mnemonic) override;

    virtual JUB_RV MnemonicToSeed(const std::string &passphrase, const std::string &mnemonic, uchar_vector &vSeed,
                                  void (*progress_callback)(JUB_UINT32 current, JUB_UINT32 total)) override;
    virtual JUB_RV MnemonicToMiniSecret(const std::string &passphrase, const std::string &mnemonic, uchar_vector &vSeed,

                                        void (*progress_callback)(JUB_UINT32 current, JUB_UINT32 total));
    virtual JUB_RV SeedToMasterPrivateKey(const uchar_vector &seed, const JUB_ENUM_CURVES &curve, std::string &xpub,
                                          std::string &xprv) override;
